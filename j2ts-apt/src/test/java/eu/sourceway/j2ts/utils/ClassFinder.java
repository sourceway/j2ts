...
public class ClassFinder {
	...
	private final static Comparator<URL> URL_COMPARATOR =
	        new Comparator<URL>() {
	...
	private void include(String name, File file, Map<URL, String> map) {
		...
		for (File dir : dirs) {
			try {
				...
				map.put(new URL("file://" + dir.getCanonicalPath()),
				    name + dir.getName());
	...
	private List<Class<?>> findSubclasses(Map<URL, String> locations,
	        String searchingPackageName) {
	...
	private List<Class<?>> findSubclasses(URL location, String packageName,
	        String searchingPackageName) {
					...
					for (String file : files) {
						...
						if (file.endsWith(".class")) {
							...
							try {
								...
								if (packageName
								    .startsWith(searchingPackageName)) {
	...
	public static void main(String[] args) {
		...
		if (v != null && v.size() > 0) {
			for (Class<?> cls : v) {
				System.out.println(cls + " in " + (finder != null
				        ? String.valueOf(finder.getLocationOf(cls)) : "?"));
			}
			...
			System.out
			    .println("No subclasses in package " + args[0] + " found.");
		}
	}
}
